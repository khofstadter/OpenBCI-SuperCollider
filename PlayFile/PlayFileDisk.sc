...
PlayFileDisk {
	...
	*new {|target, addAction= \addToHead, channels= #[1, 2, 4]|
		^super.new.initPlayFileDisk(target, addAction, channels);
	}
	initPlayFileDisk {|argTarget, argAddAction, argChannels|
...
PlayFileDiskStereo : PlayFileDisk {
	prMix {
		^{|snd| Splay.ar(snd)};
	}
}
PlayFileDiskQuad : PlayFileDisk {
	prMix {
